...
const DA83_SCHEMA_FILE = 'GAEB_DA_XML_83_3.3_2021-05.xsd';
const DA83_SCHEMA_PATH = path.join(__dirname, 'schemas', DA83_SCHEMA_FILE);
const DA83_TARGET_NAMESPACE = 'http://www.gaeb.de/GAEB_DA_XML/DA83/3.3';
...
function buildGaebDescription(lv) {
  const parts = [];
  ...
  if (Array.isArray(lv.module)) {
    for (const modul of lv.module) {
      if (typeof modul?.text === 'string' && modul.text.trim()) {
        parts.push(modul.text.trim());
      }
    }
  }
...
function splitGaebTextForStructure(description) {
  const normalized = String(description || '').replaceAll('\r\n', '\n').trim();
  if (!normalized) {
    return { outlineText: '', detailText: '' };
  }

  const lines = normalized.split('\n');
  const firstNonEmptyIndex = lines.findIndex((line) => line.trim().length > 0);

  if (firstNonEmptyIndex >= 0 && lines.length > firstNonEmptyIndex + 1) {
    const outlineText = lines[firstNonEmptyIndex].trim();
    const detailText = lines.slice(firstNonEmptyIndex + 1).join('\n').trim();
    return {
      outlineText,
      detailText: detailText || outlineText,
    };
  }

  // Fallback ohne Zeilenumbruch: ersten Satz als Outline, Rest als Detail.
  const sentenceRegex = /^(.+?[.!?])(?:\s+|$)([\s\S]*)$/;
  const sentenceMatch = sentenceRegex.exec(normalized);
  if (sentenceMatch) {
    const outlineText = sentenceMatch[1].trim();
    const detailText = sentenceMatch[2].trim();
    return {
      outlineText,
      detailText: detailText || outlineText,
    };
  ...
  return {
    outlineText: normalized,
    detailText: normalized,
  };
}

function buildGaebXmlString(steuerungLv, abnahmeLv) {
  const items = [
    { rNoPart: '01', rNoIndex: '01', description: buildGaebDescription(steuerungLv) },
    { rNoPart: '02', rNoIndex: '01', description: buildGaebDescription(abnahmeLv) },
  ];

  const itemXml = items
    .map((item) => {
      const splitText = splitGaebTextForStructure(item.description);
      return [
        '      <Item>',
        `        <RNoPart>${item.rNoPart}</RNoPart>`,
        `        <RNoIndex>${item.rNoIndex}</RNoIndex>`,
        '        <Description>',
        '          <CompleteText>',
        '            <OutlineText>',
        `              <OutlTxt>${escapeXml(splitText.outlineText)}</OutlTxt>`,
        '            </OutlineText>',
        '            <DetailText>',
        `              <Text>${escapeXml(splitText.detailText)}</Text>`,
        '            </DetailText>',
        '          </CompleteText>',
        '        </Description>',
        '      </Item>',
  ...
  return [
    ...
    '  <GAEBInfo>',
    '    <Version>3.3</Version>',
    '    <VersDate>2020-01</VersDate>',
    '  </GAEBInfo>',
    '  <BoQ>',
    '    <BoQInfo>',
    '      <Name>Leistungsverzeichnis</Name>',
    '    </BoQInfo>',
    '    <BoQBody>',
    itemXml,
    '    </BoQBody>',
...
function parseX83RequestPayload(req) {
  ...
  if (typeof req.query?.payload === 'string' && req.query.payload.trim()) {
    try {
      ...
      // Invalid payload query falls back to query-mapped data.
    }
  }

  return {
    projekt: {
      id: req.query.projektnummer || req.query.projektId || req.query.id || '',
      name: req.query.projektname || req.query.projektName || req.query.bauvorhaben || '',
    },
    anlage: {
      bezeichnung: req.query.leistung || '',
    },
  };
}

function handleX83Export(req, res) {
  try {
    ...
    const filename = buildX83Filename(x83Model.lvMeta);
    ...
    const statusCode = e && Number.isInteger(e.statusCode) ? e.statusCode : 500;
    console.error('[Export X83] ' + (e?.message || e));
    res.status(statusCode).json({ error: e?.message || 'X83 Export fehlgeschlagen' });
  }
}

function buildMinimalX83TestModel() {
  return {
    lvMeta: {
      projektId: 'TEST_X83',
      projektName: 'ORCA Test',
    ...
    item: {
      ...
      langtext: 'Lieferung und Montage einer Testposition.',
      menge: 1,
      einheit: 'St',
    },
  };
...
function buildTestInlineTextElements(text, indent) {
...
function buildTestMlText(tagName, text, indent) {
  return [
    `${indent}<${tagName}>`,
    buildTestInlineTextElements(text, `${indent}  `),
    `${indent}</${tagName}>`,
  ].join('\n');
}

function buildTestOutlTxt(text, indent) {
  return [
    `${indent}<OutlTxt>`,
    `${indent}  <TextOutlTxt>`,
    buildTestInlineTextElements(text, `${indent}    `),
...
function buildTestDetailTxt(text, indent) {
  return [
    `${indent}<DetailTxt>`,
    `${indent}  <Text>`,
    buildTestInlineTextElements(text, `${indent}    `),
...
function normalizeTestRNoIndex(value) {
...
function buildMinimalX83TestXml(model) {
  // Minimalstruktur fuer ORCA-Basistest in DA83/3.3, jetzt an die XSD-Struktur angepasst.
  return [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<GAEB xmlns="http://www.gaeb.de/GAEB_DA_XML/DA83/3.3">',
    '  <GAEBInfo>',
    ...
    '    <DP>83</DP>',
    '    <AwardInfo>',
    `      <Cur>${escapeXml(model.lvMeta.waehrung)}</Cur>`,
    '    </AwardInfo>',
    '    <BoQ ID="TEST_BOQ_01">',
    '      <BoQInfo>',
    `        <Name>${escapeXml(model.lvMeta.lvBezeichnung)}</Name>`,
    `        <LblBoQ>${escapeXml(model.lvMeta.projektName)}</LblBoQ>`,
    `        <Date>${new Date().toISOString().slice(0, 10)}</Date>`,
    ...
    '      </BoQInfo>',
    '      <BoQBody>',
    `        <BoQCtgy ID="TEST_CTG_01" RNoPart="${escapeXml(model.title.titleNo)}">`,
    buildTestMlText('LblTx', model.title.titleName, '          '),
    '          <BoQBody>',
    '            <Itemlist>',
    `            <Item ID="TEST_ITEM_01_01" RNoPart="${escapeXml(model.item.ozPart1)}" RNoIndex="${escapeXml(normalizeTestRNoIndex(model.item.ozPart2))}">`,
    `              <Qty>${escapeXml(model.item.menge)}</Qty>`,
    `              <QU>${escapeXml(model.item.einheit)}</QU>`,
    '              <Description>',
    '                <CompleteText>',
    buildTestDetailTxt(model.item.langtext, '                  '),
    '                  <OutlineText>',
    buildTestOutlTxt(model.item.kurztext, '                    '),
    '                  </OutlineText>',
    '                </CompleteText>',
    '              </Description>',
    '            </Item>',
...
function handleX83TestExport(req, res) {
  try {
    const testModel = buildMinimalX83TestModel();
    const testXml = buildMinimalX83TestXml(testModel);

    res.setHeader('Content-Type', 'application/xml; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="TEST_ORCA_IMPORT.X83"');
    res.send(testXml);
  } catch (e) {
    const statusCode = e && Number.isInteger(e.statusCode) ? e.statusCode : 500;
    console.error('[Export X83 Test] ' + (e?.message || e));
    res.status(statusCode).json({ error: e?.message || 'X83 Test Export fehlgeschlagen' });
  }
}

function validateXmlAgainstXsd(xmlContent, schemaPath) {
  if (!fs.existsSync(schemaPath)) {
    return {
      isValid: false,
      errors: [`Schema nicht gefunden: ${schemaPath}`],
      validator: 'powershell-dotnet',
    };
  }

  const tempXmlPath = path.join(os.tmpdir(), `boq-x83-${crypto.randomUUID()}.xml`);

  try {
    fs.writeFileSync(tempXmlPath, xmlContent, 'utf8');

    const escapedXmlPath = tempXmlPath.replaceAll("'", "''");
    const escapedSchemaPath = schemaPath.replaceAll("'", "''");
    const validationScript = [
      "$ErrorActionPreference = 'Stop'",
      `$xmlPath = '${escapedXmlPath}'`,
      `$schemaPath = '${escapedSchemaPath}'`,
      '$errors = New-Object System.Collections.Generic.List[string]',
      '$settings = New-Object System.Xml.XmlReaderSettings',
      '$settings.ValidationType = [System.Xml.ValidationType]::Schema',
      `$null = $settings.Schemas.Add('${DA83_TARGET_NAMESPACE}', $schemaPath)`,
      '$handler = [System.Xml.Schema.ValidationEventHandler]{ $script:errors.Add(("{0}: {1}" -f $args[1].Severity, $args[1].Message)) }',
    ...
    const result = spawnSync(
      'powershell',
      ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', validationScript],
      { encoding: 'utf8' }
    );
    ...
    if (result.status === 0 && stdoutLines.includes('VALID')) {
      return {
        isValid: true,
        errors: [],
        validator: 'powershell-dotnet',
    ...
    return {
      isValid: false,
      errors: [...stdoutLines, ...stderrLines],
      validator: 'powershell-dotnet',
    };
  } finally {
    try {
...
function handleX83TestSchemaValidation(req, res) {
  try {
    const testModel = buildMinimalX83TestModel();
    const testXml = buildMinimalX83TestXml(testModel);
    const validation = validateXmlAgainstXsd(testXml, DA83_SCHEMA_PATH);
    ...
    const statusCode = e && Number.isInteger(e.statusCode) ? e.statusCode : 500;
    console.error('[Validate X83 Test] ' + (e?.message || e));
    res.status(statusCode).json({ error: e?.message || 'X83 Test Validierung fehlgeschlagen' });
  }
}
